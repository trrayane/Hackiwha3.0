    ...
    for model in TEXT_MODELS:
        for attempt in range(3):
            try:
                ...
                # 404 (model not available) or 429 (this model's quota/rate
                # limit) → try the next model in the fallback chain rather than
                # failing outright. Only surface if every model is exhausted.
                last_exc = exc
                if getattr(exc, "code", None) in (404, 429):
                    break
                raise
    raise last_exc
