...
export const LANGUAGE_OPTIONS: { value: Language; label: string }[] = [
  { value: "ar-darija", label: "Algérien" },
...
export async function deleteAccount(password: string) {
  return request<{ message: string }>("/auth/me", { method: "DELETE", body: { password } });
}
...
export interface ReferenceAudioOut {
  id: string;
  jingle_id: string;
...
async function uploadReferenceAudioFile(
  jingleId: string,
  file: File,
  method: "POST" | "PUT"
): Promise<ReferenceAudioOut> {
  ...
  const res = await fetch(`${API_V1}/jingles/${jingleId}/reference-audio`, {
    method,
    headers,
    body: formData,
  });
  ...
  if (!res.ok) {
    throw new ApiError(res.status, extractErrorMessage(payload?.detail, `request failed (${res.status})`), payload?.detail);
  }
  return payload as ReferenceAudioOut;
}

/** Uploads the first reference audio for a jingle (409 if one already exists — use replaceReferenceAudio instead). */
export async function uploadReferenceAudio(jingleId: string, file: File) {
  return uploadReferenceAudioFile(jingleId, file, "POST");
}

/** Replaces the existing reference audio for a jingle. */
export async function replaceReferenceAudio(jingleId: string, file: File) {
  return uploadReferenceAudioFile(jingleId, file, "PUT");
}

export async function getReferenceAudio(jingleId: string) {
  return request<ReferenceAudioOut>(`/jingles/${jingleId}/reference-audio`);
}

export async function deleteReferenceAudio(jingleId: string) {
  return request<void>(`/jingles/${jingleId}/reference-audio`, { method: "DELETE" });
}
...
/** First call for a given voice/language generates and caches a short spoken
 * sample server-side; later calls just return the cached audio_url. */
export async function getVoicePreview(voiceName: string, language: Language) {
  return request<{ audio_url: string }>(
    `/voices/${encodeURIComponent(voiceName)}/preview?language=${encodeURIComponent(language)}`
  );
}
