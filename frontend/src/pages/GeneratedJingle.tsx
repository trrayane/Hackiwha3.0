...
import '../App.css';
import type { GeneratedVariantOut } from '../lib/api';
import {
  ArrowLeft,
  Moon,
  Sun,
...
interface GeneratedJingleResultProps {
  ...
  variant: GeneratedVariantOut | null;
  onBack: () => void;
...
export default function GeneratedJingleResult({
  ...
  const togglePlay = () => {
    ...
    if (!audioRef.current) audioRef.current = new Audio(variant.audio_url);
    if (isPlaying) {
  ...
  const durationLabel = variant?.duration_seconds
    ? `0:${String(Math.round(variant.duration_seconds)).padStart(2, '0')}`
    : '—';
    ...
    <div style={{
      ...
      <main style={{
        ...
        <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '20px', flexGrow: 1 }}>
              ...
              <button
                ...
                style={{
                  ...
                  color: colors.white,
                  cursor: variant?.audio_url ? 'pointer' : 'default',
                  opacity: variant?.audio_url ? 1 : 0.5,
                ...
                {waveformBars.map((height, i) => (
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor: colors.primaryAccent,
                      borderRadius: '4px',
                      opacity: variant?.audio_url ? 1 : 0.25
                    }}
                  />
                ))}
              </div>
          ...
          {!variant && (
            <div style={{ textAlign: 'center', color: colors.secondaryText, marginBottom: '24px' }}>
              No audio variant available yet.
            </div>
          )}
